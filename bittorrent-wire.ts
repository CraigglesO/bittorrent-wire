...
const debug      = require("debug")("bittorrent-wire"),
...
function Wire(infoHash: string | Buffer, myID: string | Buffer, options?: Options) {
    Duplex.call(this);
    const self = this;
...
Wire.prototype._push = function(payload: Buffer) {
  // TODO: upate keep alive timer here.
...
Wire.prototype.sendBitfield = function(bitfield: string) {
...
Wire.prototype.sendRequest = function(payload: Buffer, count: number) {
...
Wire.prototype.sendPiece = function(piece: Buffer) {
  this._debug("sending piece");
  this._push(piece);
};
// cancel: <len=0013><id=8><index><begin><length>
Wire.prototype.sendCancel = function(index: number, begin: number, length: number) {
...
Wire.prototype._nextAction = function(length: number, action: Function) {
...
// piece: <len=0009+X><id=7><index><begin><block>
Wire.prototype._onPiece = function(index: number, begin: number, block: Buffer) {
  const self = this;
  process.nextTick(() => {
    self.blockCount--;
...
Wire.prototype.createExtensions = function(metadataSize: number, torrentInfo: Info, myID?: string) {
  this.createUTmetadata(metadataSize, torrentInfo);
  this.createUTpex(myID);
};
Wire.prototype.createUTmetadata = function(metadataSize: number, torrentInfo: Info) {
  this.ext[UT_METADATA] = new UTmetadata(metadataSize, this.infoHash, torrentInfo);
};
Wire.prototype.createUTpex = function(myID?: string) {
  this.ext[UT_PEX] = new UTpex();
  if (myID)
    this.ext[UT_PEX].myID(myID);
};
Wire.prototype.sendPexPeers = function(addPeers: Array<string>, addPeers6: Array<string>, dropPeers: Array<string>, dropPeers6: Array<string>) {
  this._debug("send UT_PEX Peers");
  if (this.ext["ut_pex"]) {
    this.ext[UT_PEX].addAll(addPeers, addPeers6, dropPeers, dropPeers6);
    let prepMsg     = EXTENDED,
        msgEn       = this.ext[UT_PEX].prepMessage(),
        code        = new Buffer(1);
    prepMsg.writeUInt32BE(msgEn.length + 2, 0);
    code.writeUInt8(this.ext["ut_pex"], 0);
    let msgBuf = Buffer.concat([prepMsg, code, msgEn]);
    this._push(msgBuf);
  }
};
// extension: <len=0002+x><id=20><ext_ID><ext_msg>
Wire.prototype._onExtension = function(extensionID: number, payload: Buffer) {
  const self = this;
  if (extensionID === 0) {
    ...
    if (m["ut_metadata"]) {
      ...
      self.ext[UT_METADATA].on("meta_r", (buf: Buffer) => {
        self._debug("metadata send responce");
        let prepResponce = EXTENDED,
            code         = new Buffer(1);
        prepResponce.writeUInt32BE(buf.length + 2, 0);
        code.writeUInt8(self.ext["ut_metadata"], 0);
        let responceBuf  = Buffer.concat([prepResponce, code, buf]);
...
Wire.prototype.handleCode = function(payload: Buffer) {
  const self = this;
...
Wire.prototype.isChoked = function(): Boolean {
  return this.choked;
};

Wire.prototype.isBusy = function(): Boolean {
  return this.busy;
};
...
Wire.prototype._debug = function(...args: any[]) {
