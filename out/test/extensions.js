...
let parseTorrent = require("parse-torrent"), bencode = require("bencode"), string2compact = require("string2compact");
let id = "-EM0022-PEANUTS4AITH";
test("Metadata Request (5)", function (t) {
    t.plan(5);
    let torrent = fs.readFileSync("./screen.torrent"), torrentFile = parseTorrent(torrent), infoBuf = bencode.encode(torrentFile.info), infoLength = infoBuf.length, torrentInfo = torrentFile.info;
    let opts = {
        "metadata_handshake": {
            "ipv4": new Buffer(0),
            "ipv6": new Buffer(0),
    ...
    let wire = new bittorrent_wire_1.default(torrentFile.infoHash, id);
    let wire2 = new bittorrent_wire_1.default(torrentFile.infoHash, id, opts);
...
test("Metadata LARGE Request (5)", function (t) {
    t.plan(5);
    let torrent = fs.readFileSync("./ntbos.torrent"), torrentFile = parseTorrent(torrent), infoBuf = bencode.encode(torrentFile.info), infoLength = infoBuf.length, torrentInfo = torrentFile.info;
    let opts = {
        "metadata_handshake": {
            "ipv4": new Buffer(0),
            "ipv6": new Buffer(0),
